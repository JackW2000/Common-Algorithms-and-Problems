...
object QuickSort { 
  ...
  def quickSort(arr: Array[Int], firstIndex: Int, lastIndex: Int): Unit = { //  Quick sort: Selects an index (pivot) and sorts around it, meaning that all left items are lesser and
    ...
    if (firstIndex < lastIndex) { //  Select a value to pivot around, in this case the first index is selected
