...
object MergeSort {
  ...
  def merge(leftSplit: Array[Int], rightSplit: Array[Int], arr: Array[Int]): Unit = {
    var mainIterator = 0
    var leftIterator = 0
    var rightIterator = 0
    //  While the iterators are not at the end of each half
    while ( {
      leftIterator < leftSplit.length && rightIterator < rightSplit.length
    }) {
      //  Compare the value of the left half at the index to that of the right half
      //  at it's respective index
      if (leftSplit(leftIterator) < rightSplit(rightIterator)) { //  If the value in the left split is less than that of the right split
    ...
    while ( {
      rightIterator < rightSplit.length
    }) {
